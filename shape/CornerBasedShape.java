...
public abstract class CornerBasedShape implements Shape {
    ...
    public final Outline mo276createOutlinePq9zytI(long size, LayoutDirection layoutDirection, Density density) {
        float topStart = this.topStart.mo947toPxTmRCtEA(size, density);
        float topEnd = this.topEnd.mo947toPxTmRCtEA(size, density);
        float bottomEnd = this.bottomEnd.mo947toPxTmRCtEA(size, density);
        float bottomStart = this.bottomStart.mo947toPxTmRCtEA(size, density);
        float minDimension = Size.m3989getMinDimensionimpl(size);
        ...
        if (topStart >= 0.0f && topEnd >= 0.0f && bottomEnd >= 0.0f && bottomStart >= 0.0f) {
            return mo939createOutlineLjSzlW0(size, topStart, topEnd, bottomEnd, bottomStart, layoutDirection);
        }
