...
public interface GridCells {
    ...
    public static final class Adaptive implements GridCells {
        ...
        private Adaptive(float minSize) {
            ...
            if (Dp.m6624compareTo0680j_4(this.minSize, Dp.m6625constructorimpl((float) 0)) > 0) {
            } else {
                throw new IllegalArgumentException(("Provided min size " + ((Object) Dp.m6636toStringimpl(this.minSize)) + " should be larger than zero.").toString());
            }
        }

        @Override // androidx.compose.foundation.lazy.grid.GridCells
        public List<Integer> calculateCrossAxisCellSizes(Density $this$calculateCrossAxisCellSizes, int availableSize, int spacing) {
            int count = Math.max((availableSize + spacing) / ($this$calculateCrossAxisCellSizes.mo359roundToPx0680j_4(this.minSize) + spacing), 1);
            return LazyGridDslKt.calculateCellsCrossAxisSizeImpl(availableSize, count, spacing);
        }

        public int hashCode() {
            return Dp.m6631hashCodeimpl(this.minSize);
        }

        public boolean equals(Object other) {
            return (other instanceof Adaptive) && Dp.m6630equalsimpl0(this.minSize, ((Adaptive) other).minSize);
        }
    }
    ...
    public static final class FixedSize implements GridCells {
        ...
        private FixedSize(float size) {
            this.size = size;
            if (Dp.m6624compareTo0680j_4(this.size, Dp.m6625constructorimpl((float) 0)) > 0) {
            } else {
                throw new IllegalArgumentException(("Provided size " + ((Object) Dp.m6636toStringimpl(this.size)) + " should be larger than zero.").toString());
            }
        }

        @Override // androidx.compose.foundation.lazy.grid.GridCells
        public List<Integer> calculateCrossAxisCellSizes(Density $this$calculateCrossAxisCellSizes, int availableSize, int spacing) {
            int cellSize = $this$calculateCrossAxisCellSizes.mo359roundToPx0680j_4(this.size);
            int i = 0;
        ...
        public int hashCode() {
            return Dp.m6631hashCodeimpl(this.size);
        }

        public boolean equals(Object other) {
            return (other instanceof FixedSize) && Dp.m6630equalsimpl0(this.size, ((FixedSize) other).size);
        }
    }
}
