...
final class IntrinsicHeightNode extends IntrinsicSizeModifier {
    ...
    public long mo627calculateContentConstraintsl58MMJ0(MeasureScope $this$calculateContentConstraints_u2dl58MMJ0, Measurable measurable, long constraints) {
        int measuredHeight;
        if (this.height == IntrinsicSize.Min) {
            measuredHeight = measurable.minIntrinsicHeight(Constraints.m6578getMaxWidthimpl(constraints));
        } else {
            measuredHeight = measurable.maxIntrinsicHeight(Constraints.m6578getMaxWidthimpl(constraints));
        }
        if (measuredHeight < 0) {
            measuredHeight = 0;
        }
        return Constraints.INSTANCE.m6589fixedHeightOenEA2s(measuredHeight);
    }
