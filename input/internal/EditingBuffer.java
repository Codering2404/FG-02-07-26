...
public final class EditingBuffer {
    ...
    private EditingBuffer(AnnotatedString annotatedString, long j) {
        ...
        this.selectionStart = TextRange.m6091getStartimpl(j);
        this.selectionEnd = TextRange.m6086getEndimpl(j);
        this.compositionStart = -1;
        this.compositionEnd = -1;
        checkRange(TextRange.m6091getStartimpl(j), TextRange.m6086getEndimpl(j));
    }
    ...
    public final TextRange m1129getCompositionMzsxiRA() {
        if (hasComposition()) {
            return TextRange.m6079boximpl(TextRangeKt.TextRange(this.compositionStart, this.compositionEnd));
        }
        return null;
    }
    ...
    public final void delete(int start, int end) {
        ...
        PartialGapBuffer.replace$default(this.gapBuffer, TextRange.m6089getMinimpl(deleteRange), TextRange.m6088getMaximpl(deleteRange), "", 0, 0, 24, null);
        long newSelection = EditingBufferKt.m1132updateRangeAfterDeletepWDy79M(TextRangeKt.TextRange(this.selectionStart, this.selectionEnd), deleteRange);
        setSelectionStart(TextRange.m6091getStartimpl(newSelection));
        setSelectionEnd(TextRange.m6086getEndimpl(newSelection));
        if (hasComposition()) {
            long compositionRange = TextRangeKt.TextRange(this.compositionStart, this.compositionEnd);
            long newComposition = EditingBufferKt.m1132updateRangeAfterDeletepWDy79M(compositionRange, deleteRange);
            if (TextRange.m6085getCollapsedimpl(newComposition)) {
                commitComposition();
            } else {
                this.compositionStart = TextRange.m6089getMinimpl(newComposition);
                this.compositionEnd = TextRange.m6088getMaximpl(newComposition);
            }
        }
        this.highlight = null;
    }
    ...
    public final void m1131setHighlightK7f2yys(int type, int start, int end) {
        ...
        int clampedStart = RangesKt.coerceIn(start, 0, getLength());
        int clampedEnd = RangesKt.coerceIn(end, 0, getLength());
        this.highlight = new Pair<>(TextHighlightType.m1104boximpl(type), TextRange.m6079boximpl(TextRangeKt.TextRange(clampedStart, clampedEnd)));
    }
