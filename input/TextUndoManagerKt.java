...
public final class TextUndoManagerKt {
    ...
    public static final void recordChanges(TextUndoManager $this$recordChanges, TextFieldCharSequence pre, TextFieldCharSequence post, TextFieldBuffer.ChangeList changes, boolean allowMerge) {
        ...
        if (changes.getChangeCount() == 1) {
            long preRange = changes.mo1096getOriginalRangejx7JFs(0);
            long postRange = changes.mo1097getRangejx7JFs(0);
            if (!TextRange.m6085getCollapsedimpl(preRange) || !TextRange.m6085getCollapsedimpl(postRange)) {
                $this$recordChanges.record(new TextUndoOperation(TextRange.m6089getMinimpl(preRange), TextRangeKt.m6098substringFDrldGo(pre, preRange), TextRangeKt.m6098substringFDrldGo(post, postRange), pre.getSelection(), post.getSelection(), 0L, allowMerge, 32, null));
            }
        }
    }
