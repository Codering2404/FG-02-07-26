...
public final class BringIntoViewRequestPriorityQueue {
    ...
    public final boolean enqueue(final ContentInViewNode.Request request) {
        ...
        if (requestBounds == null) {
            ...
            continuation.resumeWith(Result.m7023constructorimpl(Unit.INSTANCE));
            return false;
        }
        ...
        if (first <= i) {
            while (true) {
                ContentInViewNode.Request r = this.requests.getContent()[i];
                ...
                if (rBounds != null) {
                    ...
                    if (!Intrinsics.areEqual(intersection, rBounds)) {
                        ...
                        if (j <= i) {
                            while (true) {
                                this.requests.getContent()[i].getContinuation().cancel(cause);
                                if (j == i) {
                                    break;
                                }
                                j++;
    ...
    public final void resumeAndRemoveAll() {
        ...
        if (i <= last) {
            while (true) {
                CancellableContinuation<Unit> continuation = this.requests.getContent()[i].getContinuation();
                Unit unit = Unit.INSTANCE;
                Result.Companion companion = Result.INSTANCE;
                continuation.resumeWith(Result.m7023constructorimpl(unit));
                if (i == last) {
                    break;
                } else {
                    i++;
    ...
    public final void cancelAndRemoveAll(Throwable cause) {
        ...
        for (int i = 0; i < size; i++) {
            ContentInViewNode.Request it = mutableVector.getContent()[i];
