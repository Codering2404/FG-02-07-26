...
public final class SelectionAdjustmentKt {
    ...
    public static final Selection.AnchorInfo updateSelectionBoundary(final SelectionLayout $this$updateSelectionBoundary, final SelectableInfo info, Selection.AnchorInfo previousSelectionAnchor) {
        ...
        Lazy anchorSnappedToWordBoundary$delegate = LazyKt.lazy(LazyThreadSafetyMode.NONE, (Function0) new Function0<Selection.AnchorInfo>() { // from class: androidx.compose.foundation.text.selection.SelectionAdjustmentKt$updateSelectionBoundary$anchorSnappedToWordBoundary$2
            ...
            public final Selection.AnchorInfo invoke() {
                return SelectionAdjustmentKt.snapToWordBoundary(info, SelectionAdjustmentKt.updateSelectionBoundary$lambda$0(currentRawLine$delegate), currentRawOffset, otherRawOffset, $this$updateSelectionBoundary.getIsStartHandle(), $this$updateSelectionBoundary.getCrossStatus() == CrossStatus.CROSSED);
            }
        });
        ...
        long previousSelectionWordBoundary = info.getTextLayoutResult().m6065getWordBoundaryjx7JFs(previousSelectionOffset);
        if (!isExpanding(info, currentRawOffset, $this$updateSelectionBoundary.getIsStartHandle())) {
            return info.anchorForOffset(currentRawOffset);
        }
        if (previousSelectionOffset == TextRange.m6091getStartimpl(previousSelectionWordBoundary) || previousSelectionOffset == TextRange.m6086getEndimpl(previousSelectionWordBoundary)) {
            return updateSelectionBoundary$lambda$1(anchorSnappedToWordBoundary$delegate);
        }
        return info.anchorForOffset(currentRawOffset);
    }
    ...
    public static final Selection.AnchorInfo snapToWordBoundary(SelectableInfo $this$snapToWordBoundary, int currentLine, int currentOffset, int otherOffset, boolean isStart, boolean crossed) {
        int start;
        int end;
        long wordBoundary = $this$snapToWordBoundary.getTextLayoutResult().m6065getWordBoundaryjx7JFs(currentOffset);
        int wordStartLine = $this$snapToWordBoundary.getTextLayoutResult().getLineForOffset(TextRange.m6091getStartimpl(wordBoundary));
        if (wordStartLine == currentLine) {
            start = TextRange.m6091getStartimpl(wordBoundary);
        ...
        int wordEndLine = $this$snapToWordBoundary.getTextLayoutResult().getLineForOffset(TextRange.m6086getEndimpl(wordBoundary));
        if (wordEndLine == currentLine) {
            end = TextRange.m6086getEndimpl(wordBoundary);
        } else if (currentLine >= $this$snapToWordBoundary.getTextLayoutResult().getLineCount()) {
    ...
    private static final Selection.AnchorInfo anchorOnBoundary(SelectableInfo $this$anchorOnBoundary, boolean crossed, boolean isStart, int slot, BoundaryFunction boundaryFunction) {
        ...
        long range = boundaryFunction.mo1314getBoundaryfzxv0v0($this$anchorOnBoundary, offset);
        return $this$anchorOnBoundary.anchorForOffset(isStart ^ crossed ? TextRange.m6091getStartimpl(range) : TextRange.m6086getEndimpl(range));
    }
