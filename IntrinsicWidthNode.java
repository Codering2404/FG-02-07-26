...
final class IntrinsicWidthNode extends IntrinsicSizeModifier {
    ...
    public long mo627calculateContentConstraintsl58MMJ0(MeasureScope $this$calculateContentConstraints_u2dl58MMJ0, Measurable measurable, long constraints) {
        ...
        if (this.width == IntrinsicSize.Min) {
            measuredWidth = measurable.minIntrinsicWidth(Constraints.m6577getMaxHeightimpl(constraints));
        } else {
            measuredWidth = measurable.maxIntrinsicWidth(Constraints.m6577getMaxHeightimpl(constraints));
        }
        if (measuredWidth < 0) {
            measuredWidth = 0;
        }
        return Constraints.INSTANCE.m6590fixedWidthOenEA2s(measuredWidth);
    }
